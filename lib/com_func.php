...
function getCombooffersbyid($conn, $language_id, $loc_id, $combo_offer_id)
{
    $sql_query = "SELECT combo_offer_id, combo_offer_name_$language_id AS combo_offer_name, total_price, total_product_count, active_status 
                  FROM combo_offers 
                  WHERE loc_id = ? AND combo_offer_id = ?
...
function get_badge_details($conn_super, $client_name)
{
    // Prepare the SQL query with a placeholder for the client_id parameter
    $sql_restaurant_details = "SELECT badge_min_order_count, other_company_ordercharges_percent 
                               FROM client_restaurant_detail 
                               WHERE client_name = ? LIMIT 1";

    // Initialize a prepared statement
    $stmt = mysqli_prepare($conn_super, $sql_restaurant_details);

    // Bind the client_id parameter to the statement
    mysqli_stmt_bind_param($stmt, 's', $client_name); // 'i' indicates that the parameter is an integer
...
function OrderCheckAndAwardBadge($conn, $loc_id, $min_order_count = 20, $other_company_charges_percent) 
{
    // Current datetime
    $today_datetime = date("Y-m-d H:i:s");
    
    // Define base month for comparison (previous month)
    $base_month_start = date('Y-m-01', strtotime('-1 month'));
    $base_month_end = date('Y-m-t 23:59:59', strtotime('-1 month'));
    
    // Query to get the base month order count
    $sql_base_query = "SELECT COUNT(order_id) as base_month_count 
                       FROM orders 
                       WHERE loc_id = ? 
                       AND orders_status_id = 6 
                       AND order_datetime BETWEEN ? AND ?";
    
    $stmt_base = $conn->prepare($sql_base_query);
    $stmt_base->bind_param("iss", $loc_id, $base_month_start, $base_month_end);
    $stmt_base->execute();
    $result_base = $stmt_base->get_result();
    $base_month_count = $result_base->fetch_assoc()['base_month_count'];
    
    // Query to get the current month order count and total order amount
    $sql_current_query = "SELECT COUNT(order_id) as current_month_count, SUM(final_payable_amount) as total_order_amount 
                          FROM orders 
                          WHERE loc_id = ? 
                          AND orders_status_id = 6 
                          AND MONTH(order_datetime) = MONTH(CURRENT_DATE()) 
                          AND YEAR(order_datetime) = YEAR(CURRENT_DATE())";
    
    $stmt_current = $conn->prepare($sql_current_query);
    $stmt_current->bind_param("i", $loc_id);
    $stmt_current->execute();
    $result_current = $stmt_current->get_result();
    $current_month_data = $result_current->fetch_assoc();
    $current_month_count = $current_month_data['current_month_count'];
    $total_order_amount = $current_month_data['total_order_amount'];

    // Check if the current month order count meets the minimum order count requirement
    if ($current_month_count < $min_order_count) {
        return [
            'badge' => 'No Badge',  // No badge awarded if the minimum order count is not met
            'total_orders' => $current_month_count,
            'savings' => 0 // No savings since no significant order volume
        ];
    }

    // Calculate the savings based on the other company's charges
    $savings = ($total_order_amount * $other_company_charges_percent) / 100;

    // Determine which badge to award based on the current month count compared to the base month count
    $badge = '';
    if ($current_month_count >= $base_month_count * 1.00) {
        $badge = 'Platinum';
    } elseif ($current_month_count >= $base_month_count * 0.75) {
        $badge = 'Gold';
    } elseif ($current_month_count >= $base_month_count * 0.50) {
        $badge = 'Silver';
    } elseif ($current_month_count >= $base_month_count * 0.25) {
        $badge = 'Bronze';
    }

    // Update the badge status in the database if a badge is awarded
    /*if ($badge) {
        $sql_update = "UPDATE location_badges 
                       SET badge_awarded = ?, awarded_date = ? 
                       WHERE loc_id = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $badge, $today_datetime, $loc_id);
        $stmt_update->execute();
    }*/

    // Return the awarded badge, total order count, and savings
    return [
        'badge' => $badge,
        'total_orders' => $current_month_count,
        'savings' => $savings
