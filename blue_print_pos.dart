...
class BluePrintPos {
  ...
  Future<void> _printProcess(List<int> byteBuffer) async {
    try {
      ...
      if (Platform.isAndroid) {
        print("the length is: " + byteBuffer.length.toString());
        // const int chunkSize = 1800; // Adjust this size as needed
        const int chunkSize = 8000; // Adjust this size as needed
        // int offset = 0;
        // while (offset < byteBuffer.length) {
        //   int end = (offset + chunkSize < byteBuffer.length)
        //       ? offset + chunkSize
        //       : byteBuffer.length;
        //   List<int> chunk = byteBuffer.sublist(offset, end);

        //   // _bluetoothAndroid?.writeBytes(Uint8List.fromList(byteBuffer));
        //   _bluetoothAndroid?.writeBytes(Uint8List.fromList(chunk));
        ...
        //   int end = (i + chunkSize < byteBuffer.length) ? i + chunkSize : byteBuffer.length;
        //   _bluetoothAndroid?.writeBytes(Uint8List.fromList(byteBuffer.sublist(i, end)));
        // }

        for (int offset = 0; offset < byteBuffer.length; offset += chunkSize) {
          int end = (offset + chunkSize < byteBuffer.length) ? offset + chunkSize : byteBuffer.length;
          List<int> chunk = byteBuffer.sublist(offset, end);
          _bluetoothAndroid?.writeBytes(Uint8List.fromList(chunk));
        }
